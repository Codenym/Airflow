...
upper(replace(trim(organization_name),'  ',' ')) as organization_name,
trim(ein) as ein,
upper(replace(trim(mailing_address_1),'  ',' ')) as mailing_address_1,
upper(replace(trim(mailing_address_2),'  ',' ')) as mailing_address_2,
upper(replace(trim(mailing_address_city),'  ',' ')) as mailing_address_city,
upper(replace(trim(mailing_address_state),'  ',' ')) as mailing_address_state,
...
trim(e_mail_address) as e_mail_address,
       ...
       upper(replace(trim(custodian_name),'  ',' '))                                                                  as custodian_name,
upper(replace(trim(custodian_address_1),'  ',' '))                                                             as custodian_address_1,
upper(replace(trim(custodian_address_2),'  ',' '))                                                             as custodian_address_2,
upper(replace(trim(custodian_address_city),'  ',' '))                                                          as custodian_address_city,
upper(replace(trim(custodian_address_state),'  ',' '))                                                         as custodian_address_state,
trim(custodian_address_zip_code)                                                                    as custodian_address_zip_code,
trim(custodian_address_zip_ext )                                                                    as custodian_address_zip_ext,

       upper(replace(trim(contact_person_name),'  ',' '))                                                              as contact_person_name,
upper(replace(trim(contact_address_1),'  ',' '))                                                               as contact_address_1,
upper(replace(trim(contact_address_2),'  ',' '))                                                               as contact_address_2,
upper(replace(trim(contact_address_city),'  ',' '))                                                            as contact_address_city,
upper(replace(trim(contact_address_state),'  ',' '))                                                           as contact_address_state,
trim(contact_address_zip_code)                                                                      as contact_address_zip_code,
trim(contact_address_zip_ext)                                                                      as contact_address_zip_ext,

         upper(replace(trim(business_address_1),'  ',' '))                                                              as business_address_1,
upper(replace(trim(business_address_2),'  ',' '))                                                              as business_address_2,
upper(replace(trim(business_address_city),'  ',' '))                                                           as business_address_city,
upper(replace(trim(business_address_state),'  ',' '))                                                          as business_address_state,
...
trim(business_address_zip_ext)                                                                     as business_address_zip_ext,
       ...
       upper(replace(trim(pre_elect_type),'  ',' ')) as pre_elect_type,
       if(pre_or_post_elect_date = '', NULL,
          strptime(pre_or_post_elect_date, '%Y%m%d')::date)                                    as pre_or_post_elect_date,
       upper(replace(trim(pre_or_post_elect_state),'  ',' '))                                                         as pre_or_post_elect_state,
